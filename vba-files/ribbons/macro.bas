...
Sub EnviarEmail()
    ...
    k = 1
    Do While sh_layout.Cells(k, "B").Value <> "DATA"
        k = k + 1
    Loop
    ...
    Do While sh_capa.Cells(lin_bd + i, "B").Value <> ""
        ...
        num_prod = 0
        Do While sh_capa.Cells(lin_bd + i, "C").Value = sh_capa.Cells(lin_bd + i + num_prod, "C").Value
            num_prod = num_prod + 1
        Loop
        ...
        k = 0
        Do While sh_capa.Cells(lin_bd + k, "O").Value <> ""
            'Se nome da tabela bd for igual a o nome da tabela de e-mails...
            If sh_capa.Cells(lin_bd + k, "O").Value = sh_capa.Cells(lin_bd + i, "C").Value Then
                'Configura a variável destino com o e-mail desejado
                If sh_capa.Cells(lin_bd + k, "Q").Value <> "x" Then
                    GoTo proximo
                End If
                destino = sh_capa.Cells(lin_bd + k, "P").Value
                Exit Do
            End If
            k = k + 1
        Loop
        ...
        'Nome da loja
        sh_layout.Cells(4, "C").Value = sh_layout.Cells(lin_tabela, "C").Value & ","
        ...
        'Assunto o e-mail
        outMail.Subject = sh_capa.Cells(8, "M").Value
        'Destinatário
        outMail.To = destino
        'Cópia
        outMail.CC = sh_capa.Cells(9, "M").Value
        'Cópia oculta
        outMail.BCC = sh_capa.Cells(10, "M").Value
        ...
        For k = 0 To num_prod - 1
            sh_capa.Cells(lin_bd + i + k, "J").Value = "Enviado"
        Next k
